...
const UI_TEXT = {
  ...
  page_salir_sub: {
    es: "Santiago no compite en tamaño con Madrid o Barcelona, pero sí en ambiente: al ser ciudad universitaria y de peregrinación, mezcla cada noche a estudiantes, vecinos, turistas y peregrinos del Camino en un público muy diverso. No existe un \"barrio gay\" como Chueca —la ciudad es demasiado pequeña para eso—, pero sí una cultura nocturna donde parejas del mismo sexo, estudiantes internacionales y peregrinos conviven con naturalidad en la mayoría de locales; tanto la web oficial de turismo como guías especializadas la destacan como una ciudad especialmente acogedora para el colectivo LGTBIQ+, aunque con una escena específica reducida (Bloom y O Curruncho son los referentes más claros). Fuera de esos locales \"de ambiente\", sitios como Pub Atlántico o Embora tienen un público igual de heterogéneo — en Compostela no hace falta buscar un bar concreto para sentirte cómodo. Filtra abajo por lo que te apetezca esta noche.",
    en: "Santiago doesn't compete with Madrid or Barcelona in size, but it does in atmosphere: as both a university city and a pilgrimage destination, every night it mixes students, locals, tourists, and Camino pilgrims into a wonderfully varied crowd. There's no \"gay district\" like Madrid's Chueca — the city is simply too small for that — but there is a nightlife culture where same-sex couples, international students, and pilgrims mix naturally in most venues; both the official tourism website and specialized guides highlight it as an especially welcoming city for the LGBTQI+ community, even if the specific scene is small (Bloom and O Curruncho are the clearest go-to spots). Outside those explicitly \"ambiente\" venues, places like Pub Atlántico or Embora draw an equally mixed crowd — in Compostela you don't need to seek out one particular bar to feel comfortable. Filter below by whatever you're in the mood for tonight."
  },
...
function getLang() {
  return localStorage.getItem("boda_lang") || "es";
}
